...
static struct peripheral_status_state get_state(const zmk_event_t *_eh) {
    // zmk_split_bt_peripheral_is_connected() is a BLE-only API and doesn't
    // exist with the ESB split transport, which has no equivalent link
    // status signal yet. Always show connected until that exists.
